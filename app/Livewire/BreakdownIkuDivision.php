...
class BreakdownIkuDivision extends Component
{
    ...
    public function mount($year = null, $month = null, $department = null)
    {
        ...
        $this->selectedMonth = str_pad($month ?? $currentDate->month, 2, '0', STR_PAD_LEFT);
        $this->selectedYear = $year ?? $currentDate->year;
        $this->selectedPeriod = $this->selectedYear . '-' . $this->selectedMonth;
    ...
    public function fetchDepartments()
    {
        ...
        if ($user->role === 'division') {
            $this->departments = Department::where('division_id', $user->division_id)
                ->select('department_id', 'department_name')
                ->get();
        } elseif ($user->role === 'director') {
            $this->departments = Department::whereHas('division', function ($query) use ($user) {
                $query->where('director_id', $user->director_id);
            })->select('department_id', 'department_name')->get();
        } else {
            $this->departments = Department::select('department_id', 'department_name')->get();
        }
    }
    ...
    public function fetchEvaluations()
    {
        ...
        $selectedDepartment = $this->selectedDepartment ?? null;
        ...
        ", [$selectedYear, $selectedMonth, $selectedDepartment, $selectedDepartment]);
    }

    public function render()
    {
        return view('livewire.breakdown-iku-division', [
